...
<?php
	...
	if (!$con)
  {
	...
	$query = "SELECT ID, TITLE, DESCRIPTION, PRICE, CURRENCY, CAT_ID, ICON, WEBSITE, EMAIL, PHONE, LAT, LONGITUDE, FB_ID, DATE_FORMAT(onDate, '%d %b %Y') as onDate, FB_NAME FROM ZIP_AD_MASTER WHERE IS_REPORTED_ABUSED = 0 AND IS_REVOKED = 0";
